            ...
            for (int j =0; j < pictureBox1.Height; j+=3)
            {
                for (int i = 0; i < pictureBox1.Width; i+=3)
                {
                    ...
                    if (Math.Pow(radius, 2) - (Math.Pow(i - pictureBox1.Width / 2, 2) + Math.Pow(j - pictureBox1.Height / 2, 2)) > 0)
                    {
                        ...
                        // 'Angle''s mean is angle of picturebox's plane about (x-y plane) 
                        // the pictureBox1 Center Position is half of PictureBox's Height and Width 

                        angle = Math.Atan(Convert.ToDouble(j - pictureBox1.Height / 2) / Convert.ToDouble(i - pictureBox1.Width / 2));
                        // angle < 0 : Second and Forth quadrant 
                        // angle > 0 : First and Third quadrant
                        if (angle < 0.0f)
                        {
                            angle += 2.0f * Math.PI;
                        }
                        // '1' remainder : odd number of Section in x-y plane is divided 16 sections by angle
                        // '2' remainder : even number of Section in x-y plane is divided 16 sections by angle 
                        if (Convert.ToInt32((16 * angle /(Math.PI * 2))) % 2 == 0)
                        {
                        ...
                        {
                            Pen mypen = new Pen(Color.FromArgb(nBright, nBright, 0, nBright), 0.1f);
                            g.DrawLine(mypen, i, j, i + 1, j);
                        }
                    }
                }
